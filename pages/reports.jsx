            ...
            <div className="overflow-hidden rounded-3xl bg-white shadow-lg">
              <div className="px-8 py-6" style={{ backgroundColor: "#D97706" }}>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <h2 className="text-2xl font-black text-white">Cashbook Summary</h2>
                </div>
                <p className="text-sm text-amber-100 ml-11">Real totals from fee, expense, and payroll records.</p>
              </div>

              <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-4">
                <SummaryTile label="Total fee collected" value={formatCurrency(data.totalCollected)} tone="text-emerald-600" />
                <SummaryTile label="Today's collection" value={formatCurrency(data.todayCollection)} tone="text-blue-600" />
                <SummaryTile label="Total expenses" value={formatCurrency(data.expenses)} tone="text-red-600" />
                <SummaryTile label="Pending salary" value={formatCurrency(data.salaryPending || data.salaries)} tone="text-orange-600" />
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {(data.monthlyCollections || []).map((item) => (
                      <tr key={item.month_key || item.month} className="hover:bg-slate-50 transition">
                        <td className="px-5 py-4 font-semibold text-slate-900">{item.month}</td>
                        <td className="px-5 py-4 text-sm font-semibold text-emerald-600">{formatCurrency(item.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {(data.monthlyCollections || []).length === 0 && (
                  <div className="p-8 text-center text-sm text-slate-500">
                    No monthly cashbook data found.
                  </div>
                )}
              </div>
            </div>
